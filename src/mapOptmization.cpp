...
class mapOptimization : public ParamServer {
  public:
    ...
    vector<pcl::PointCloud<PointType>::Ptr> other_cloud_keyframes_;
    
    pcl::PointCloud<PointType>::Ptr cloudKeyPoses3D;
    pcl::PointCloud<PointTypePose>::Ptr cloudKeyPoses6D;
    pcl::PointCloud<PointTypePose>::Ptr gps_poses_;
    ...
    pcl::PointCloud<PointType>::Ptr cloud_plane_;
    pcl::PointCloud<PointType>::Ptr cloud_other_;
    ...
    pcl::PointCloud<PointType>::Ptr laserCloudOtherFromMap;
    pcl::PointCloud<PointType>::Ptr laserCloudCornerFromMapDS;
    pcl::PointCloud<PointType>::Ptr laserCloudSurfFromMapDS;
    pcl::PointCloud<PointType>::Ptr laserCloudOtherFromMapDS;
...
//    int laserCloudOtherFromMapDSNum = 0;
    int laserCloudCornerLastDSNum = 0;
    int laserCloudSurfLastDSNum = 0;
    int laserCloudOtherLastDSNum = 0;
    ...
    std::vector<int>        local_map_points_record_;
    ...
    mapOptimization() {
        ...
        plane_tracker_.SetIncrementalFitting(is_incremental_fitting);
        plane_tracker_.SetPlaneLeastInliers(plane_least_inliers);   // do not keep old planes
    }
    
    void allocateMemory() {
        cloudKeyPoses3D.reset(new pcl::PointCloud<PointType>());
        cloudKeyPoses6D.reset(new pcl::PointCloud<PointTypePose>());
        gps_poses_.reset(new pcl::PointCloud<PointTypePose>());
        ...
        if (is_extract_large_planes) {
            cloud_plane_.reset(new pcl::PointCloud<PointType>());
            cloud_other_.reset(new pcl::PointCloud<PointType>());
        }
        ...
        if (is_extract_large_planes) {
            laserCloudOtherFromMap.reset(new pcl::PointCloud<PointType>());
            laserCloudOtherFromMapDS.reset(new pcl::PointCloud<PointType>());
        }
    ...
    void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr &msgIn) {
        ...
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval) {
            ...
            if (is_extract_large_planes) {
                D_RECORD_TIME_START;
                plane_tracker_.ExtractLargePlanes(laserCloudSurfLastDS,
                    is_incremental_fitting ? transformTobeMapped : nullptr);
                cloud_plane_ = plane_tracker_.CurrCloudPlane();
                cloud_other_ = plane_tracker_.CurrCloudOther();
                D_RECORD_TIME_END("ExtractLargePlanes");
            }
            
            D_RECORD_TIME_START;
            scan2MapOptimization();
            D_RECORD_TIME_END("scan2MapOptimization");
            ...
            plane_tracker_.SetCurrTransWl(transformTobeMapped);
        }
    }
    
    void gpsHandler(const nav_msgs::Odometry::ConstPtr &gpsMsg) {
        PointTypePose gps_pose;
        gps_pose.x = gpsMsg->pose.pose.position.x;
        gps_pose.y = gpsMsg->pose.pose.position.y;
        gps_pose.z = gpsMsg->pose.pose.position.z;
        gps_pose.time = gpsMsg->header.stamp.toSec();
        static bool is_first_gps = true;
        static double first_gps_time = 0;
        if (is_first_gps) {
            first_gps_time = gps_pose.time;
            is_first_gps = false;
        }
        gps_pose.time -= first_gps_time;
        gps_poses_->push_back(gps_pose);
...
//        unused = system((std::string("mkdir -p ") + saveMapDirectory).c_str());
    ...
    void updateInitialGuess() {
        ...
        // initialization
        if (cloudKeyPoses3D->points.empty()) {
            ROS_ASSERT(!cloudInfo.odomAvailable);
        ...
        for (int i = 0; i < (int) cloudToExtract->size(); ++i) {
            ...
            if (laserCloudMapContainer.find(thisKeyInd) != laserCloudMapContainer.end()) {
                ...
                *laserCloudCornerFromMap += laserCloudMapContainer[thisKeyInd].first;
                if (is_extract_large_planes) {
                    *laserCloudOtherFromMap += laserCloudMapContainer[thisKeyInd].second;
                } else {
                ...
                *laserCloudCornerFromMap += laserCloudCornerTemp;
                if (is_extract_large_planes) {
                    pcl::PointCloud<PointType> laserCloudOtherTemp = *transformPointCloud(other_cloud_keyframes_[thisKeyInd],    &cloudKeyPoses6D->points[thisKeyInd]);
                    *laserCloudOtherFromMap   += laserCloudOtherTemp;
                    laserCloudMapContainer[thisKeyInd] = make_pair(laserCloudCornerTemp, laserCloudOtherTemp);
                } else {
...
//        laserCloudCornerFromMapDSNum = laserCloudCornerFromMapDS->size();
        ...
        if (is_extract_large_planes) {
            downSizeFilterSurf.setInputCloud(laserCloudOtherFromMap);
            downSizeFilterSurf.filter(*laserCloudOtherFromMapDS);
        } else {
...
//            laserCloudSurfFromMapDSNum = laserCloudSurfFromMapDS->size();
    ...
    void extractSurroundingKeyFrames() {
        ...
        // radius: surroundingKeyframeSearchRadius = 50m
        ...
        // Leaf size: surroundingKeyframeDensity = 2.0m
        ...
        D_RECORD_TIME_END("construct local map");
        local_map_points_record_.emplace_back(is_extract_large_planes ?
                                              laserCloudOtherFromMapDS->size() :
                                              laserCloudSurfFromMapDS->size());
    }
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    void surfOptimizationOurs() {
        transPointAssociateToMap = trans2Affine3f(transformTobeMapped);
    
        const int plane_n_pts = (int) cloud_plane_->size();
        const int other_n_pts = (int) cloud_other_->size();
    
        const int n_planes = plane_tracker_.NumPlanes();
        auto &last_planes = plane_tracker_.LastPlanes();
#pragma omp parallel for num_threads(numberOfCores)
        for (int i = 0; i < plane_n_pts; i++) {
            PointType pointOri, pointSel, coeff;
            std::vector<int> pointSearchInd;
            std::vector<float> pointSearchSqDis;
        
            pointOri = cloud_plane_->points[i];
            pointAssociateToMap(&pointOri, &pointSel);
        
            float min_dist = std::numeric_limits<float>::max();
            for (auto &candidate_plane : last_planes) {
                float dist_sign = Point2PlaneDistSigned(pointSel, candidate_plane.coef);
                float dist = fabs(dist_sign);
                if (dist < point_to_plane_thresh) {
                    if (dist < min_dist) {
                        min_dist = dist;
                        float s = 1 - 0.9 * dist / sqrt(sqrt(pointOri.x * pointOri.x
                                 + pointOri.y * pointOri.y + pointOri.z * pointOri.z));
                        coeff.x = s * candidate_plane.coef[0];
                        coeff.y = s * candidate_plane.coef[1];
                        coeff.z = s * candidate_plane.coef[2];
                        coeff.intensity = s * dist_sign;
                        if (s > 0.1) {
...
#pragma omp parallel for num_threads(numberOfCores)
        for (int i = 0; i < other_n_pts; i++) {
            PointType pointOri, pointSel, coeff;
            std::vector<int> pointSearchInd;
            std::vector<float> pointSearchSqDis;
    
            pointOri = cloud_other_->points[i];
            pointAssociateToMap(&pointOri, &pointSel);
            ...
            if (pointSearchSqDis[4] < 1.0) {
                for (int j = 0; j < 5; j++) {
                    matA0(j, 0) = laserCloudOtherFromMapDS->points[pointSearchInd[j]].x;
                    matA0(j, 1) = laserCloudOtherFromMapDS->points[pointSearchInd[j]].y;
                    matA0(j, 2) = laserCloudOtherFromMapDS->points[pointSearchInd[j]].z;
                }
                ...
                for (int j = 0; j < 5; j++) {
                    if (fabs(pa * laserCloudOtherFromMapDS->points[pointSearchInd[j]].x +
                             pb * laserCloudOtherFromMapDS->points[pointSearchInd[j]].y +
                             pc * laserCloudOtherFromMapDS->points[pointSearchInd[j]].z + pd) > 0.2) {
                        planeValid = false;
                        break;
                    }
                }
                if (planeValid) {
                    ...
                    if (s > 0.1) {
                        ...
                        laserCloudOriSurfVec[idx] = pointOri;
                        coeffSelSurfVec[idx] = coeff;
                        laserCloudOriSurfFlag[idx] = true;
                    }
                }
            }
        }
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    void scan2MapOptimization() {
        ...
        if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum && laserCloudSurfLastDSNum > surfFeatureMinValidNum) {
            ...
            D_RECORD_TIME_END("Corner construct kd-tree");
            D_RECORD_TIME_START;
            if (is_extract_large_planes) {
                kdtreeSurfFromMap->setInputCloud(laserCloudOtherFromMapDS);
                D_RECORD_TIME_END("Other construct kd-tree");
            } else {
                kdtreeSurfFromMap->setInputCloud(laserCloudSurfFromMapDS);
                D_RECORD_TIME_END("Surf construct kd-tree");
            }
            ...
            for (int iterCount = 0; iterCount < 30; iterCount++) {
                ...
                D_RECORD_TIME_START;
                cornerOptimization();
                D_RECORD_TIME_END("cornerOptimization");
                D_RECORD_TIME_START;
                if (is_extract_large_planes) {
                    surfOptimizationOurs();
                    D_RECORD_TIME_END("surfOptimizationOurs");
                } else {
                    surfOptimization();
                    D_RECORD_TIME_END("surfOptimization");
                }
                ...
                bool suc;
                D_RECORD_TIME_START;
                suc = LMOptimization(iterCount);
                D_RECORD_TIME_END("LMOptimization");
                if (suc)
                    break;
            }
    ...
    void saveKeyFramesAndFactor() {
        ...
        if (is_extract_large_planes) {
            pcl::PointCloud<PointType>::Ptr cloud_other(new pcl::PointCloud<PointType>());
            pcl::copyPointCloud(*cloud_other_, *cloud_other);
            other_cloud_keyframes_.push_back(cloud_other);
        }
    ...
    void correctPoses() {
        if (!cloudKeyPoses3D->points.empty() && aLoopIsClosed) {
            // clear map cache
            laserCloudMapContainer.clear();
            // clear path
        ...
        if (!lastIncreOdomPubFlag) {
            ...
            if (cloudInfo.imuAvailable) {
                if (std::abs(cloudInfo.imuPitchInit) < 1.4) {
                    ...
                    imuQuaternion.setRPY(cloudInfo.imuRollInit, 0, 0);
                    tf::Matrix3x3(transformQuaternion.slerp(imuQuaternion, imuWeight)).getRPY(
                            rollMid, pitchMid,yawMid);
                    roll = rollMid;
    ...
    void PrintLocalMapRecord() {
        if (local_map_points_record_.empty()) {
            return;
        }
        std::cout << "mean local map points cnt: "
                  << std::accumulate(local_map_points_record_.begin(),
                                     local_map_points_record_.end(), 0.0) /
                     double(local_map_points_record_.size())
                  << ", n: " << local_map_points_record_.size() << std::endl;
    }
};
...
int main(int argc, char **argv) {
    ...
    Timer::Instance().PrintAll();
    MO.PrintLocalMapRecord();
    return 0;
}
